...
export default function OneDaySewaPage() {
  const sewa = ALL_SEWAS.find(s => s.id === '1-day-pind-daan') || ALL_SEWAS[0];
  ...
  return (
    <div className="min-h-screen bg-parchment-100 py-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-12">
        ...
        <div className="space-y-4">
          <h2 className="font-serif text-2xl font-bold text-mahogany-900">
            Aapke Liye Kya Arrange Hoga?
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {sewa.whatWeArrange.map((item, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-parchment-50 border border-gold-600/30 flex items-center gap-3">
                <CheckCircle2 className="w-5 h-5 text-saffron-600 flex-shrink-0" />
                <span className="text-xs font-medium text-mahogany-900">{item}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Aapka Din Kaise Guzrega (Timeline) */}
        <div className="space-y-6">
          <h2 className="font-serif text-2xl font-bold text-mahogany-900">
            Aapka Din Kaise Guzrega?
          </h2>

          <div className="space-y-4 relative before:absolute before:inset-0 before:left-5 before:w-0.5 before:bg-gold-600/30">
            {sewa.timeline.map((step, idx) => (
              <div key={idx} className="relative flex items-start gap-4 pl-2">
                <div className="w-10 h-10 rounded-full bg-mahogany-900 text-gold-400 font-serif font-bold text-xs flex items-center justify-center flex-shrink-0 border-2 border-gold-500 z-10">
                  {step.step}
                </div>
                <div className="bg-parchment-50 p-5 rounded-xl border border-gold-600/30 flex-1 space-y-1">
                  <h3 className="font-serif font-bold text-base text-mahogany-900">
                    {step.title}
                  </h3>
        ...
        <div className="p-8 rounded-2xl bg-mahogany-950 text-parchment-100 border border-gold-600/40 text-center space-y-4">
          <h3 className="font-serif text-2xl font-bold text-parchment-50">
            Is Sewa Ko Shuru Karein
          </h3>
          <p className="text-xs text-parchment-300 max-w-lg mx-auto">
            Gaya Ji arrival date aur family members details submit karein. Coordinator team aapse sampark karke sab confirm karegi.
          </p>
